...
- **A binding expression can crash the client outright.** A probe carrying `(#count / 19)` and `('' + #count)` — a division and an empty string literal — closed the game on opening the screen; the same probe without those two forms ran. `#count` is a string; `/` on a float binding such as durability works ([S14](./S14-custom-item-carriers.md)). `%` is not an operator and asserts `Invalid expression`.
