...
| **Host** | claiming a root, the carriers and inputs it offers, `allocate`, `emit` (JSON UI for a placed carrier, the mount, the router), the runtime loops | component names, fibers |
| **JSON UI** | the render pack vocabulary; the emitted per-screen documents | anything about the server |

The IR is the seam the design rests on: above it nothing dispatches on a host, below it nothing dispatches on a component. Today's code crosses that seam in both directions — `components/Button.ts` carries a form writer and a container cell claim, `validateForm`/`validateContainer` list component types by hand, `nodes/button.ts` knows the transport item — and that is what the rewrite removes.
...
open(screen)                 the host detects the open (interact, show(), ...)
  |- session(owner)          player-owned or entity-owned fibers (today's Owner)
  |- render -> tree
