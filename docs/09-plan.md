...
| 5 | **Consumers, then delete legacy** — *`List max` and the conditional sugar done* | ~~`List max`~~ (the int carrier: `max` row slots compiled, one count entry, a gate per row enumerating the counts that show it as string equalities; the rows are a `stack_panel`'s children, so hidden rows collapse and a scroll over a list reaches exactly the real rows — see *Measured while building List*); ~~`Tabs`~~ (landed with phase 3); ~~conditional sugar~~ (`{cond && <X/>}` and element ternaries desugar to carried `visible` at build — [03-ir](./03-ir.md), `ui-compiler/lib/sugar.ts`, applied by compile and bundler alike); the guides filter emits IR and pages compile as screens with a replicated reference table; config and the addon list on compiled `List max` screens; the interpreter fallback and its decoders deleted; pack minor | 2 weeks |
| 6 | **Build flow** — *the `core` filter done* | ~~the single `core` filter~~ (one Regolith entry running the six stages in order, namespace under `shared`); CLI template; docs site pages replace this folder | 4 days |
...
**Carried `visible` is probe-detected and snapshot-carried.** It could not be declared: `withControl` stamps `visible: true` on every element and the inherit pass forces `false` down a hidden subtree, so the prop's presence says nothing about the author. The build probes it instead — an element whose `visible` a state flip moved is carried, subtree ROOTS only (descendants flip with their ancestor by inheritance, and one gate hides them all) — and hands the runtime the element ordinals through the compiled snapshot in `ui.generated.ts`, the same walk on both sides. On the action form the bool is one entry ('0'/'1'); on the modal, one bare label row; the emitted control is wrapped in a gate reading it, seeded with the build's value. The chest refuses a live visible at build: it has no carrier for one.

**The modal's rows carry its live values.** A modal screen's channels ride `custom_form` label rows, never `form_buttons` — `compileFormScreen` branches on the root, and text emission is collection-parameterised. This also removed a latent double-address: a modal with live text used to be given a `form_buttons` entry it could never read.

**The snapshot is the contract.** `registerCompiledScreen` takes the build's snapshot beside the title: the visible ordinals (load-bearing — the runtime re-marks the same elements, keeping the entry numbering identical to the bake) and the claim fingerprint plus baked strings (`debug`). `render(Screen, player, { debug: true })` diffs every present against it and warns on a baked string that drifted or a shape that no longer matches — the runtime half of the probe's guard. A live string past its reservation is not reportable: `<Text>` slices to `maxLength` at element creation.

## Open questions

- **Guides — decided; own-addon half built.** Every page compiles into the owning addon's RP as a form-action screen, and a page's buttons navigate by opening another page's key. *Built:* the guides filter writes one generated `*.screen.tsx` per page plus the home index (`guide_<page id>`, `guide_home`) into the scripts folder, each three lines — the manifest imported relatively, a factory from `@bedrock-core/guides` (`guidePageScreen` / `guideHomeScreen`), the default export — so discovery, compile, snapshot registration, bundling and the conditional sugar are the ordinary screen path with nothing special-cased. Navigation is `render()` of the next screen for the pressing player (`openGuide(ns, player)` lands the guide; every view callback now carries the `PressEvent`). The ui-compiler entry loads the addon's i18n bundle before compiling, so a localized `<Text>` is detected and measured as its real default-locale string — that applies to every compiled screen, not only guides. The compiled home folds its categories with `<Disclosure>` (built 2026-09-03, measured in game): a toggle header whose faces are baked per state, a stack of rows whose `visible` reads the toggle's `#toggle_state`, and the column holding it emitted as a stack of pitched rows — so a fold reflows everything below on the client with nothing reaching script, and a scroll over that column follows. One deliberate difference from `createGuide` remains: every screen is compiled for the widest audience — a gated guide keeps rendering through `createGuide` until gating rides a carried visible. Every compiled guide screen is baked at a 300×200 canvas (inside the 320×210 the smallest UI scale draws) and the form gate now mounts every compiled canvas centred, since the form's content box is 0×0 at screen centre for a compiled title. *Remaining:* the header breadcrumb bakes as a resolved default-locale string (should be a rawtext the client resolves); cross-addon presentation from a replicated **reference** table (home key + per page the key each entry opens — never the prose) so any realm shows another addon's guide with title = key, `selection` → next key, no script of the owner involved; `GuidesRegistry` shrinks from the manifest to that table; the config app's Guide route moves onto it.
- **Config — pick one.** The problem: realm A (the host election) draws addon B's config screen, but a compiled screen's shape is fixed when A is built, and B's schema only arrives at runtime over sync. Two ways out:
  ...
  *Built 2026-09-03 (`@bedrock-core/config/compiled`, screen `config_scope`, compiled into every addon that lists it under ui-compiler's `screens` setting; measured in game):* a section whose entries all fit its rows (≤ 12 rows, labels ≤ 32, dropdowns ≤ 8 options, no lists/multiselect/sub-sections) opens in the compiled editor, rendered as a screen of its own that swaps into the running app and closes on Save; anything else keeps the serialized editor. Numbers go through the input. Each compiled screen hosts its own dropdown popups (`<ns>_popups`), since the library container's host only resolves for screens mounted inside it.

  *Shape of option 1, as scoped 2026-09-03.* A compiled modal (`<Form>`) whose body is `<List max={N}>`; every row is one generic config entry: a live label (`Text maxLength`), and the native fields a row may need — toggle, input, dropdown, slider — each behind a carried visibility, so the schema decides per present which one shows. The modal host already offers every carrier this needs (bool, int, text, field); the `enum` carrier it declares is sugar over a text row compared with `=` per field, and is not required to start. Everything the fields need per present — a slider's range, a dropdown's options, defaults — already travels in the native form data the runtime builds, so it is per present by construction; only the decoration is compiled. Two things a fixed shape cannot follow and must be capped or decided: the compiled slider bakes its step count (a generic row either uses a fixed step count with the runtime snapping to the schema step, or numbers go through an input); the compiled dropdown bakes its popup box (cap the option count). Rows beyond the count are hidden by the list gate but still constructed (rule 12) — their native fields exist in the form the runtime sends, which is what keeps every compiled field's collection row present. The addon list is the same shape with one more carrier: a live texture (an image whose `texture` is a `#texture` view binding over a text entry, the interpreter's own idiom) for each addon's icon and thumbnail.
