...
import {
  dropdownFace, inputFace, type OptionFace, optionParts, type OptionState, selectFace, sliderFace,
  type TextStyle, toggleFace,
} from '../../faces';
import { boxOf, FULL, num, str } from '../utils/shared';
import type { Control, ControlEntry } from '../../jsonui';
import type { Rect } from '../../ir';
import type { LowerContext, NodeBase, NodeDefinition } from '../utils/types';
 ...
 *
 * A control the pack places owns a collection entry when it carries a literal
 * `collection_index` under a host declaring the collection, and its own
 * `collection_details` binding. The widget then sits exactly where the layout
 * put it.
 *
 * That is what replaces the factory. Vanilla's own instantiates one control
 * per row and lays the rows out at its own pitch, which leaves a compiled
 * screen no say in where a field goes. Placing them costs less
...
export const inlineOptionFace = (option: InlineOption, state: InlineOptionState): ControlEntry[] =>
  optionParts({ ...optionFaceOf(option, ''), state });

/** One placed option, as the face layer takes it. */
const optionFaceOf = (option: InlineOption, name: string): OptionFace => ({
  name,
  rect: option.rect,
...
export const fieldDefinition: NodeDefinition<FieldNode> = {
  ...
  //
  // One face per kind, because that is what they are: a switch, a track, a box
  // of text, a closed dropdown, a column of options. The look is all this node
  // has — everything that makes it answerable belongs to the screen.
  face(node): ControlEntry {
    const box = boxOf(node);
    const style: TextStyle = { fontScaleFactor: node.scale };
    ...
    switch (node.field) {
      case MODAL_TOGGLE_SLOT_TYPE:
        return toggleFace({
          ...
          off: node.faces['$off'] ?? '',
          on: node.faces['$on'] ?? '',
          checked: node.initial.on === true,
        });

      case MODAL_SLIDER_SLOT_TYPE:
        return sliderFace({
          ...box,
          ...node.faces['$track'] === undefined ? {} : { track: node.faces['$track'] },
          ...node.faces['$thumb'] === undefined ? {} : { thumb: node.faces['$thumb'] },
      ...
      case MODAL_INPUT_SLOT_TYPE:
        return inputFace({
          ...box,
          ...node.faces['$static_texture'] === undefined ? {} : { background: node.faces['$static_texture'] },
          text: node.initial.text ?? '',
          placeholder: node.initial.placeholder ?? '',
          style,
        });
      ...
      case MODAL_INLINE_SELECT_SLOT_TYPE:
        return selectFace({
          ...box,
          ...node.faces['$static_texture'] === undefined ? {} : { background: node.faces['$static_texture'] },
          selected: node.selected ?? 0,
          options: (node.options ?? []).map((option, index) => optionFaceOf(option, `option_${String(index)}`)),
        });
