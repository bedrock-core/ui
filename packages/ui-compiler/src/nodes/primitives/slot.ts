...
export interface SlotNode extends NodeBase {
   ...
   * only undo a move a tick later, never veto one, and its {@link role} does
   * not apply. A foreign slot carries its own flag on {@link source}.
   */
  interactive: boolean;
  /**
   * A collection other than the screen's own. Present makes the slot foreign:
   * `slot` and `role` do not apply, and the runtime never touches it.
   */
  source?: SlotSource;
}
