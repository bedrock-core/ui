...
import { listFace, type Row } from '../../faces';
...
export const listDefinition: NodeDefinition<ListNode> = {
  ...
  face(node, ctx): ControlEntry {
    const rows: Row[] = node.rows.map((row, index): Row => {
      const next = node.rows[index + 1];

      return {
        name: row.name,
        control: ctx.emitNode({ ...row, rect: { ...row.rect, y: 0 } }),
        span: next === undefined ? row.rect.height : next.rect.y - row.rect.y,
      };
    });

    return listFace({ ...boxOf(node), rows, shown: node.initial });
  },
};
