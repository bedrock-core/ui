...
import { SLOT_TYPE } from '../../components/Slot';
...
const ACTION_ONLY_INTERACTIVE_TYPES = new Set<string>([BUTTON_TYPE]);
...
/** Controls that bind to a container the form does not have, by the name the author wrote. */
const CONTAINER_ONLY_TYPES = new Map<string, string>([
  [SLOT_TYPE, 'Slot'],
  [SLOT_GRID_TYPE, 'SlotGrid / PlayerInventory / Hotbar'],
]);

/**
 * Enforce the form restrictions on a built tree. This is the runtime backstop
 * behind the type-level guards on `<Form>` and `<Container>`; it catches
 * dynamically-built or type-escaped trees before the presenter picks a backend.
 *
 * Rules:
 *  - A `<Container>` is a compiled screen and never a form; the same goes for the
 *    controls that only exist inside one (`Slot`, `PlayerInventory`, `Hotbar`).
 *  - A modal-only control (`Form.Toggle`/`Slider`/`Dropdown`/`Input`) must live inside
 *    a `<Form>`.
 *  - Inside a `<Form>`: no ActionForm-only interactive control (`Button`) and no
 *    nested `<Form>`.
 ...
 *
 * @throws ModalFormError on a modal violation, ContainerScreenError on a container one.
 */
...
function walk(node: JSX.Node, insideModal: boolean): void {
  ...
  if (typeof type === 'string') {
    ...
    const containerOnly = CONTAINER_ONLY_TYPES.get(type);

    if (containerOnly !== undefined) {
      throw new ContainerScreenError(
        `\`${containerOnly}\` only exists in a container screen. Wrap the screen in `
