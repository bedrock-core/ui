...
/**
 * What a handler may be called with. A player who left, an entity that died, a
 * block whose chunk unloaded or a stack the engine already emptied would throw
 * the moment the handler touched them, so every argument is checked for its
 * type id and validity first, and a handler that cannot be called safely is
 * skipped.
 */
...
export const validHost = (value: ScreenHost | undefined): value is ScreenHost =>
