 ...
 * with the cell that hides a button's transport item for the tick the runtime
 * takes to reclaim it.
 */
export const PlayerInventory: FunctionComponent<ControlProps> = (props: ControlProps): JSX.Element =>
  SlotGrid({ collection: 'inventory_items', columns: 9, rows: 3, hideOwned: true, ...props });
...
export const Hotbar: FunctionComponent<ControlProps> = (props: ControlProps): JSX.Element =>
  SlotGrid({ collection: 'hotbar_items', columns: 9, rows: 1, hideOwned: true, ...props });
