 ...
 * - `key()` — client-resolved `.lang` keys via `localizationKey`, measured
 ...
 */
import { Panel, Text, usePlayer, useState, type JSX } from '@bedrock-core/ui';
...
export function I18nDemo(): JSX.Element {
  ...
  const { t, raw, locale } = i18n.forPlayer(player);

  const rerender = (): void => { setTick(n => n + 1); };

  return (
    <Card flexDirection={'column'} padding={12} gap={spacing.sm}>
      <Text font={'minecraftTen'} scale={2}>{t($ => $.demo.title)}</Text>
      <Text>{`${fontColor.muted}${t($ => $.demo.intro)}`}</Text>

      <Divider />

      {/* ── t(): server-resolved strings ────────────────────────────────── */}
      <Text shadow={true}>{`§e${t($ => $.demo.section.server)}`}</Text>
      {/* Selector form, interpolating a vanilla string t() resolved first. */}
      <Text>{t($ => $.demo.bought, { item: t($ => $.vanilla.item.apple.name), price: 64 })}</Text>
      {/* Dot-string form — typed identically to the selector. */}
      <Text>{`${fontColor.muted}${t('demo.apple', { name: t('vanilla.item.apple.name') })}`}</Text>

      <Divider />

      {/* ── Plurals ─────────────────────────────────────────────────────── */}
      <Text shadow={true}>{`§e${t($ => $.demo.section.plurals)}`}</Text>
      ...
      <Text shadow={true}>{`§e${t($ => $.demo.section.client)}`}</Text>
      {/* Own key: painted from .lang per player, wrapped from bundle metrics. */}
      <Text localizationKey={key($ => $.test.long)} wordBreak={'break-word'} maxLines={2} overflow={'ellipsis'} />
      {/* Vanilla key: resolves on the client, never shipped in this RP. */}
      <Text localizationKey={key($ => $.vanilla.item.apple.name)} />
      {/* raw() without arguments ≡ the bare key — still client-resolved. */}
      <Text localizationKey={raw($ => $.vanilla.item.apple.name)} />
      ...
      <Text localizationKey={raw($ => $.demo.bought, { item: 'Apple', price: 9 })} />
      <Text localizationKey={raw($ => $.vanilla.multiplayer.player.joined, [player.name])} />
      {/* Nested translate as an argument — painted: filled server-side too. */}
      <Text localizationKey={raw($ => $.demo.bought, { item: raw($ => $.vanilla.item.apple.name), price: 2 })} />
      <Button onPress={(): void => { player.sendMessage(raw($ => $.demo.bought, { item: 'Apple', price: 5 })); }}>
        {`§a${t($ => $.demo.action.sendRaw)}`}
      </Button>
      ...
      <Button onPress={(): void => { player.sendMessage(raw($ => $.vanilla.multiplayer.player.joined, [player.name])); }}>
        {`§a${t($ => $.demo.action.sendRawVanilla)}`}
      </Button>
      ...
      <Button onPress={(): void => { player.sendMessage(raw($ => $.demo.stock, { count })); }}>
        {`§a${t($ => $.demo.action.sendRawPlural, { count })}`}
      </Button>
      ...
      <Button onPress={(): void => { player.sendMessage(raw($ => $.demo.bought, { item: raw($ => $.vanilla.item.apple.name), price: 3 })); }}>
        {`§a${t($ => $.demo.action.sendRawNested)}`}
      </Button>

      <Divider />

      {/* ── Locale chain & override ─────────────────────────────────────── */}
      <Text shadow={true}>{`§e${t($ => $.demo.section.locale)}`}</Text>
      <Text>{t($ => $.demo.locale, { locale })}</Text>
      <Text>{`${fontColor.muted}${t($ => $.demo.pinned, { value: i18n.forLocale('es_ES').t($ => $.demo.title) })}`}</Text>
      <Panel flexDirection={'row'} gap={spacing.sm}>
        <Button variant={'secondary'} onPress={(): void => { i18n.setLocale(player, 'es_ES'); rerender(); }}>
          {`§0${t($ => $.demo.action.overrideEs)}`}
        </Button>
        <Button variant={'contrast'} onPress={(): void => { i18n.clearLocale(player); rerender(); }}>
          {t($ => $.demo.action.clearOverride)}
        </Button>
      </Panel>
    </Card>
  );
