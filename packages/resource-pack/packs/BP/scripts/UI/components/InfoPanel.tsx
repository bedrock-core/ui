...
export const InfoPanel: FunctionComponent = (): JSX.Element => {
  ...
  return (
    <Panel width={192} height={140} x={10} y={310}>
      ...
      <Text width={192} height={15} x={20} y={362} value={`§cHealth: §f${health}`} />
      <Text width={192} height={15} x={20} y={379} value={`§bMode: §f${player.getGameMode()}`} />
      <Text width={192} height={15} x={20} y={396} value={`§dDim: §f${player.dimension.id}`} />
      <Text width={192} height={15} x={20} y={413} value={`§aLevel: §f${player.level}`} />
    </Panel>
  );
};
